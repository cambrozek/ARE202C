...
output_9 <- lm(complier ~ age*j1 + age*j3 + education*j1 + education*j3 + capital*j1 + capital*j3 + land*j1 + land*j3, data=subset_nic_rct_2)
output_10 <- glm(complier ~ age*j1 + age*j3 + education*j1 + education*j3 + capital*j1 + capital*j3 + land*j1 + land*j3, data=subset_nic_rct_2, family=binomial(link="probit"))

q6 <- sjt.lm(output_7, output_8, output_9, output_10, robust = TRUE, p.numeric = TRUE)
